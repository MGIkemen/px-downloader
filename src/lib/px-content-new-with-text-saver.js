...
export default class PxContentNew extends EventEmitter {
	...
	/*get information of this page from Ajax*/
		getPageinfo(){
			let info="";
			/*get url of this page*/
			if (this.url.href!=null){
				info=`Work.pageUrl: ${this.url.href}`;
			}
			/*get id of this page*/
			if (this.macro.id != null){
				info+=`\nWork.ID: ${this.macro.id}	`;
			}
			/*get title of this page*/
			if (this.macro.title != null){
				info+=`Work.Title: ${this.macro.title}`;
			}
			/*get User name*/
			if (this.macro.userName != null){
				info+=`\nWork.UserName: ${this.macro.userName}	`;
			}
			/*get User id and homepage*/
			if (this.macro.userId != null){
				info+=`User.ID: ${this.macro.userId}	User.Homepage: https://www.pixiv.net/member.php?id=${this.macro.userId}`;
			}
			//get datetime
			info+=`\nWork.Date:\nDate.Year: ${this.macro.YYYY} Date.Month: ${this.macro.MM}	Date.Day:${this.macro.DD}\nWork.Time: ${this.macro.hh}:${this.macro.mm}`;
			//get Page count
			if(this.page === "multiple"){
				info+=`\nWork.Pagecount: ${this.illustData.pageCount}`;
			}
			//get tags of work
			if(this.illustData.hasOwnProperty("tags")){
				let tags_parent=this.illustData.tags;
				if(tags_parent.hasOwnProperty("tags")){
					info+=`\nWork.Tags:\n`;
					let i=0;
					for(i=0;i<tags_parent.tags.length;i++){
						info+=`Tag${i}: ${tags_parent.tags[i].tag}	`;
					}
				}
			}else{
				let tags=this.document.querySelectorAll("li.tag");
				if(tags!==null && tags.length>0){
					info+=`\nWork.Tags:\n`;
					let i=0
					for(i=0;i<tags.length;i++){
						info+=`Tag${i}: ${tags[i].querySelector("a.text").textContent}	`
					}
				}
				
			}
			//get caption of work
			if(this.illustData.hasOwnProperty("illustComment")){
				info+=`\nWork.Caption: \n${this.illustData.illustComment}`;
			}else{
				let caption=this.document.querySelector("p.caption");
				if(caption!==null){
					info+=`\nWork.Caption: \n${caption.textContent}`;
				}
			}	
	...
	getInfoFilename(options){
			let filename;

			if (options.isMultiFile==true) {
				filename = `${this.replaceInfoMacro(options.multiFilename,options.isMultiFile)}.txt`;
			} else {
				filename = `${this.replaceInfoMacro(options.singleFilename,options.isMultiFile)}.${options.ext}.txt`;
			}
	...
	//replace information filename
	replaceInfoMacro(filename,isMultiFile){
            const macro = {};
			if(isMultiFile===true){
				macro.page ="meta";
				macro.page2 ="meta";
			...
			Object.assign(macro, this.macro);
			for (const key of Object.keys(macro)){
                filename = filename.split("${" + key + "}").join(g.escape(macro[key], !0));
			}
            return filename;
	}
    async downloadPixiv() {
    ...
    async downloadIllust(options) {
		...
		/*Download Pageinfo*/
		await this.download({
            blob: this.pageinfo.infofile,
            filename: this.getInfoFilename({ singleFilename: options.singleFilename, ext: this.getExt(imageBlob),isMultiFile: false }),
            conflictAction: options.conflictAction
        });
    }
    ...
    async downloadManga(options) {
		...
		await this.download({
            blob: this.pageinfo.infofile,
            filename: this.getInfoFilename({ multiFilename: options.multiFilename, isMultiFile: true }),
            conflictAction: options.conflictAction
        });
    }
    ...
    async downloadUgoiraZip(options) {
		...
		await this.download({
            blob: this.pageinfo.infofile,
            filename: this.getInfoFilename({ singleFilename: options.singleFilename, ext: this.getExt(zipBlob),isMultiFile: false }),
            conflictAction: options.conflictAction
        });
    }
    ...
    async downloadNovel(options) {
        ...
        const pageDomParser = new DOMParser();
        const pageDocument = pageDomParser.parseFromString(pageText, "text/html");
		//Save novel info to the text file.
        const textBlob = new Blob([pageinfo.info+"\n"+pageDocument.querySelector("#novel_text").textContent.replace(/\r\n|\r|\n/g, "\r\n")], { type: "text/plain" });
